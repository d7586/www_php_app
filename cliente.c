...
#define SERVER_ADDRESS "192.168.18.11" /* IP servidor */
#define PORT 4040

/* Secuencias de prueba */
//char buf_tx[] = "El cliente se está conectando";
//char buf_rx[100]; /* recibe al buffer */
char mensaje_cliente[1024];
char mensaje_servidor[1024];
...
int main() /* Este cliente se conecta, envía un texto y se desconecta. */
{
    ...
    */
    ...
    printf("Ingrese una mensaje para enviar al server: ");
    scanf("%*c%[^\n]", mensaje_cliente);
    send(sockfd,mensaje_cliente, 1024,0);

    recv(sockfd, mensaje_servidor, 1024,0);
