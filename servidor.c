...
int main(int argc, char* argv[]) /* los argumentos de entrada no se utilizan */
{
    ...
    int len_rx, len_tx = 0;
    //char buff_tx[BUF_SIZE] = "Servidor activo";/* longitud recibida y enviada en bytes*/
    char buff_rx[BUF_SIZE]; /* buffers de recepción */
    char mensaje_recibido[1024];
    ...
    while(1)
    {
        ...
        else
        {
            while (1) /* Lo primero que hará el servidor es detectar si hay datos del cliente*/
            {
                ...
                len_rx = recv(connfd, mensaje_recibido, 1024,0);
                if (len_rx == -1) {
                ...
                } else if (len_rx == 0) /* if length is 0 client socket closed, then exit */
                {
                    ...
                    //printf("[SERVER]: %s \n", buff_rx);
                    send(connfd, mensaje_recibido, 1024, 0);
